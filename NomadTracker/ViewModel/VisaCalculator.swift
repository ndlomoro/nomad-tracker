...
class VisaCalculator {
    ...
    private func calculateRolling90_180Status(
        ...
        let schengenStays = allStays.filter { s in
            let daysAgo = Stay.elapsedDays(from: s.entryDate, to: today)
            return daysAgo <= 180
        }

        // Calculate total days spent in Schengen in rolling 180-day window
        var totalDaysInWindow = 0
        let windowStart = Calendar.current.date(byAdding: .day, value: -180, to: today) ?? today
        for s in schengenStays {
            let effectiveEntry = max(s.entryDate, windowStart)
            let effectiveExit = s.exitDate ?? today
            let days = Stay.elapsedDays(from: effectiveEntry, to: effectiveExit)
    ...
    func canEnterCountry(
        ...
        case .rolling90_180:
            ...
            let schengenStays = pastStays.filter { s in
                let daysAgo = Stay.elapsedDays(from: s.entryDate, to: today)
                return daysAgo <= 180
            }
